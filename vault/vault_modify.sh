...
# # Need to add kepair to server inorder to retreive our token
...
# SSH_USER="ubuntu"
# SSH_KEY="/home/ubuntu/.ssh/id_rsa" # Optional, if using passwordless login
...
# run_vault_commands() {
#     ssh -i $SSH_KEY $SSH_USER@$VAULT_SERVER << EOF
# # Initialize Vault
...
# EOF
#      scp -i $SSH_KEY $SSH_USER@$VAULT_SERVER:root_token.txt .
# }
...
# # Create a systemd service to run the Vault setup script
# sudo tee /etc/systemd/system/vault-setup.service > /dev/null <<'EOT'
# [Unit]
...
# [Service]
# Type=oneshot
# ExecStart= /home/ubuntu/vault_setup.sh

# [Install]
# WantedBy=multi-user.target
# EOT
# sudo systemctl daemon-reload  # Reload systemd to read the new service unit
# sudo systemctl enable vault-setup.service  # Enable the service to start on boot
# sudo systemctl start vault-setup.service  # Start the service immediately
